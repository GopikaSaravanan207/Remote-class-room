...
export default function StudentProgress({ role = 'staff' }: { role?: 'staff' | 'student' }) {
  ...
  // Detect aggregated attempt format (has totalQuestions) vs legacy per-question rows
  const isAggregated = progress.length > 0 && (progress[0] as any).totalQuestions !== undefined;

  const stats = isAggregated
    ? {
      total: progress.length,
      passed: progress.filter((p: any) => (p.percentage ?? p.score ?? 0) >= 50).length,
      avgScore: progress.length > 0 ? (progress.reduce((acc: any, p: any) => acc + (p.percentage ?? p.score ?? 0), 0) / progress.length).toFixed(1) : 0
    }
    : {
      total: progress.length,
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {progress.map((p: any, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-slate-900">{p.topic || p.subject}</div>
                    <div className="text-xs text-slate-500">{(p.subject && p.subject) || 'General'}</div>
                  </td>
                  <td className="px-6 py-4">
                    {isAggregated ? (
                      <div className={`flex items-center gap-1.5 text-sm font-bold ${(p.percentage ?? 0) >= 50 ? 'text-emerald-600' : 'text-red-600'}`}>
                        {(p.percentage ?? 0) >= 50 ? <CheckCircle2 size={16} /> : <XCircle size={16} />}
                        <div>{Math.round(p.percentage ?? 0)}% • {p.status || 'Needs Improvement'}</div>
                      </div>
                    ) : (
                      <div className={`flex items-center gap-1.5 text-sm font-bold ${p.score > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
