...
import { BookOpen, FileText, History, ChevronRight, PlayCircle, Globe, File, Sparkles, X, RefreshCw, Loader2, MessageCircle, Send, Edit2, Check, Save, Bell, CheckCheck } from 'lucide-react';
import Markdown from 'react-markdown';
import { useNavigate } from 'react-router-dom';
import { api } from '../api';
import { Material, Question, Progress, Notification } from '../types';
...
const groupedAiEntries = Object.entries(groupedAiMaterials).sort((a, b) => a[0].localeCompare(b[0]));
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const data = await api.student.getNotifications();
        ...
        {unreadCount > 0 && (
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-orange-100 border border-orange-200 text-orange-700 rounded-xl hover:bg-orange-200 transition-all"
        ...
        <section className="mb-8 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              {unreadCount > 0 && (
                <span className="px-2 py-0.5 bg-orange-600 text-white text-xs font-bold rounded-full">{unreadCount}</span>
              )}
            </div>
            {unreadCount > 0 && (
              <button
            ...
            {notifications.slice(0, 5).map((n) => (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  {!n.is_read && <span className="w-2 h-2 bg-orange-600 rounded-full mt-2" />}
                </div>
              </div>
            ))}
          </div>
