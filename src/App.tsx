...
const Navbar = ({ user, onLogout }: { user: User | null; onLogout: () => void }) => {
  ...
  const staffLinks = [
    ...
    { name: 'Test Results', icon: Trophy, path: '/staff/results' },
    { name: 'Progress', icon: History, path: '/staff/progress' },
  ];
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:block">
            <div className="ml-4 flex items-center md:ml-6 gap-4">
              {user ? (
                <>
<Link to="/profile" className="flex items-center gap-2 px-3 py-1 bg-slate-50 rounded-full border border-slate-200 hover:bg-orange-50 hover:border-orange-200 transition-all">
...
<div className="flex items-center px-5">
                  ...
                  <div className="ml-3">
                    <Link to="/profile" className="text-base font-medium leading-none text-slate-900 hover:text-orange-600">
                      {user.name}
                    </Link>
                    <p className="text-sm font-medium leading-none text-slate-500 capitalize">{user.role}</p>
                  </div>
                  <button
                    onClick={onLogout}
...
<Route path="/register" element={!user ? <Register /> : <Navigate to={`/${user.role}`} />} />
            ...
            <Route path="/staff/results" element={user?.role === 'staff' ? <TestResults /> : <Navigate to="/login" />} />
